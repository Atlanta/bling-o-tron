...
module.exports = {
	name: 'add',
    description: 'Add money to user\'s bank balance.',
    /**
	...
	async execute(message, args) {
        ...
        await bank.set(user, balance + amount);

        let customCurrency = await db.get('config.currency');
        message.channel.send(`Added ${amount}${customCurrency || currency} to ${userTag}'s balance ! New balance : ${amount + balance}${customCurrency || currency}`);
	},
};